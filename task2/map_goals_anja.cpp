...
std::vector<Point> getOccupancyFreePoints() {
    ...
    for (uint32_t y = 0; y < map_size_y_; ++y) {
        for (uint32_t x = 0; x < map_size_x_; ++x) {
            ...
            if (map_data_[index] == 0 && map_data_costmap_[index] ==  0) {  // Assuming 0 represents occupancy free
                Point point;
...
void prison_ring_callback(const std_msgs::String::ConstPtr& msg){
    //to do
    //if(msg ->  )
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber prison_ring_sub = n.subscribe("/prison_ring", 5, &prison_ring_callback);
    ...
    while(ros::ok()) {
        if (searching_mode_) {
            ...
            */
            ...
            primitive_timer_ += 1;
            searching_mode_ =  !(ring_position_received_ && robot_position_received_ && cylinder_finished_status_); // (face_finished_status_ && cylinder_finished_status_ && ring_finished_status_);
