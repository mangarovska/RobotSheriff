...
class Arm_Mover():
    def __init__(self):
        ...
        self.up = JointTrajectory()
        self.up.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint", "arm_wrist_flex_joint"]
        self.up.points = [JointTrajectoryPoint(positions=[0, 0, 0, 0],
                                                    time_from_start = rospy.Duration(1))]
    ...
    def update_position(self):
        ...
        if self.send_command:
            if self.user_command == 'retract':
                ...
                print('Right-ed arm!')
            elif self.user_command == 'up':
                self.arm_movement_pub.publish(self.up)
                print('Hands up!')
            else:
...
if __name__ == "__main__":
    am = Arm_Mover()
    time.sleep(.5)
    am.arm_movement_pub.publish(am.up)
    #am.update_position("up")
    #print('Retracted arm!')
